  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [IconButton(icon: Icon(Icons.check, color: Colors.black), onPressed: _kaydet)],
      ),
      body: ListView(
        children: [
  ...
  _galeridenSec() async {
    try {
      var image = await ImagePicker()
          .getImage(source: ImageSource.gallery, maxWidth: 800, maxHeight: 600, imageQuality: 80);

      setState(() {
        _secilmisFoto = File(image.path);
      });
    } catch (e) {
      print("Fixlenmesi Gerek Hata v1");
    }
  }
